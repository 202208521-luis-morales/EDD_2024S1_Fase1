MODULE BigPrinterQueueListModule
  IMPLICIT NONE
  ...
  TYPE :: BigPrinterQueue
    TYPE(Node), POINTER :: front
    TYPE(Node), POINTER :: rear
  END TYPE BigPrinterQueue

CONTAINS

  SUBROUTINE InitializeBigPrinterQueue(queue)
    TYPE(Queue), INTENT(OUT) :: queue
    queue%front => NULL()
    queue%rear => NULL()
  END SUBROUTINE InitializeBigPrinterQueue
...
END MODULE BigPrinterQueueListModule
